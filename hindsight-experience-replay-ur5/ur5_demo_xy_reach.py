...
args = get_args()
model_path = args.save_dir + args.env_name + '/July29_reach_3.pt'
...
for nTests in range(1):
    ...
    obs = np.hstack((startPos, np.zeros(7))) # wrong

    #sampling goal
    rndDisp =-SampleRange+ 2*SampleRange*np.random.random(2)
    g = startPos
    g[0:2] = list(np.asarray(startPos)[0:2] + rndDisp)
