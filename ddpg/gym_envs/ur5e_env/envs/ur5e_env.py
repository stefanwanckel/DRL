...
MIN_GOAL_COORDS = np.array([-0.42, -0.42, 0.1])
MAX_GOAL_COORDS = np.array([0.42, 0.42, 0.42])
...
class Ur5eEnv(gym.Env):
    ...
    def reset(self):
        ...
        if self.random_reset_position:
            self._force_joint_positions(self.sample_random_joint_angles())
        else :
            self._force_joint_positions(RESET_VALUES)
    ...
    def sample_random_joint_angles(self):
        # EE position must be inside min ans max goal coordinates
        random_valid_ee_point = np.random.uniform(low=MIN_GOAL_COORDS, high=MAX_GOAL_COORDS)

        jointAngles = p.calculateInverseKinematics(self.arm, 7,  random_valid_ee_point)
